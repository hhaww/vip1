...
@sedthon.on(events.NewMessage(outgoing=True, pattern=r"\.مشاكس (.*)"))
async def _(event):
    if ispay2[0] == "yes":
        isclaim.clear()
        isclaim.append("on")
        ...
        @sedthon.on(events.NewMessage(outgoing=True, pattern=r"\.حالة مشاكس"))
        async def _(event):
            if ispay2[0] == "yes":
                if "on" in isclaim:
                    await event.edit(f"مشاكس وصل لـ({trys}) من المحاولات")
                elif "off" in isclaim:
                    await event.edit("لايوجد مشاكس شغال !")
                else:
        ...
        for i in range(int(msg[0])):
            ...
            if "Available" in isav:
                ...
                try:
                    ...
                    await event.client.send_message(event.chat_id, f''- Done ↣  (@{username}) !
- By ↣ @TT_RP !'')
                    break
